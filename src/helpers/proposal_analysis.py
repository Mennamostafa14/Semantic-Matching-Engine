...
def _build_prompt(context_a: str, context_b: str) -> str:
    return f"""You are an expert at comparing research proposals.
You are given two proposals with their similarity scores and matching text passages.
Your job is to EXPLAIN the similarity, not to score it.

Return ONLY a JSON object:
{{
  "explanation": "<2-3 sentences explaining why these proposals are similar based on the passages>",
  "key_similarities": ["<first shared topic>", "<second shared topic>", "<third shared topic>"]
}}
...
def build_llm_context(proposal: dict) -> str:
    keywords = ", ".join(proposal.get("keywords", {}).get("overlap", [])[:8])

    passages = proposal.get("top_passages", [])
    passages_text = "\n".join([
        f"- [{p.get('score', 0):.1f}%]: {p.get('text', '')[:200]}"
        for p in passages[:3]
    ])

    return (
        f"Keywords: {keywords}\n"
        f"Top matching passages:\n{passages_text}"
    )
# ---------------------------------------------------------------------------
# Safe parser
# ---------------------------------------------------------------------------
...
def generate_similarity_analysis(query_proposal: dict, db_proposal: dict, generation_client):
    context_a = build_llm_context(query_proposal)
    context_b = build_llm_context(db_proposal)
